# CLAUDE.md - Streetwise

## Project Overview
Streetwise is a Seattle transportation project tracker and civic engagement platform built with Next.js 14, Supabase, and Mapbox.
...
- `src/lib/utils.ts` - Utility functions (urgency colors, formatting)
...
- Use lowercase-with-dashes for folders, PascalCase for components
...
- Component-specific: `.btn-primary`, `.card`, `.input`, `.badge-*`
- Urgency colors: `.urgency-red`, `.urgency-yellow`, `.urgency-green`, `.urgency-gray`

### Database
- All queries use Supabase client from `@/lib/supabase/client` or `@/lib/supabase/server`
- Types in `@/lib/types.ts` match database schema
- RLS policies enforce permissions - don't bypass with service role unless necessary

### Authentication
- Check auth state with `supabase.auth.getUser()` in server components
...
```typescript
// Red: This week, Yellow: Next week, Green: This month, Gray: > 1 month
import { getUrgencyColor } from '@/lib/utils';
const color = getUrgencyColor(dateString); // returns 'red' | 'yellow' | 'green' | 'gray'
```

## Database Enums
- `user_role`: 'user' | 'moderator' | 'admin'
- `project_source`: 'official_sdot' | 'official_wadot' | 'user_suggestion'
- `project_status`: 'proposed' | 'under_review' | 'planned' | 'in_progress' | 'completed' | 'rejected' | 'on_hold'
- `date_type`: 'public_meeting' | 'comment_deadline' | 'construction_start' | 'construction_end' | 'council_vote' | 'funding_deadline' | 'other'
...
## Documentation
- Project status: `docs/CONTEXT.md`
- Database schema: `docs/database/schema.md`
- Feature docs: `docs/features/[feature].md`
- Session summaries: `docs/sessions/[date]-summary.md`
...
## Important Notes
1. PostGIS extension must be enabled in Supabase for geometry columns
2. RLS is enabled on all tables - test queries as authenticated user
3. Vote/comment/follower counts are denormalized and updated via triggers
4. Map requires valid Mapbox token to render

Below is an additional document that you should use for styling and maintaining code compliance:
...
0. NEVER under any circumstances use any emojis or emoticons. You are strictly limited to text (except for ASCII designs for applicable use cases such as database designs)
